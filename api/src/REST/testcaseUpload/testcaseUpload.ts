...
export const testcaseUpload = expressRouter.post("/", [
    ...
    if (!errors.isEmpty()) {
        ...
        if (!problem) {
            ...
            if (req.user._id.toString() !== problem.author?.toString() && !(req.user.userTypes === "admin")) {
                res.status(403);
                res.end();
            } else {
                ...
                if (!zipFile) {
                    ...
                    let { success, meta, reason } = await validateAndExtractTestcaseZipFile(zipFile.data, problem._id);
                    if (!success) {
                        res.status(406);
                        res.json({
                            success: false,
